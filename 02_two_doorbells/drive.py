"""Rung 02 driver — drive() plus ring() (the doorbell primitive) and the
human doorbell (approve).

Commands: reset | start | approve | status | pending

The machine doorbell lives in farm.py; the human doorbell is `approve`.
Both end up in ring(): one function_response, one fresh run, same session.
"""
import asyncio
import os
import sys
...
async def cmd_start():
    ...
    print("[start] the trailer, with QC in the loop")
