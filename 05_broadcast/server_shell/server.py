...
class Run:
    ...
    def _bridge_call(self, f):
        ...
        if f.name == "submit_render":
            ...
            # TODO(HOOK 1): broadcast the submit. Emit a `job_submitted` event
            # (fields: job=<kind>, kind=JOB_MEDIA[...]) and a world_patch that
            # sets world/show.assets.<kind> to {"status": "rendering", "take": take}.
            # Until you do, the board never shows a job starting.
        elif f.name == "ask_human":
            self.human_call = f.id
            # TODO(HOOK 2): broadcast the pause. Emit `job_paused_need_human`
            # (job, question, call_id=f.id), patch the trailer to paused, and —
            # the part everyone forgets — emit the contract's wait signal:
            # {"type": "awaiting_action", "action": "rooms/2/reply",
            #  "match": {"call_id": f.id}}
            # Without awaiting_action the room's button stays disabled.
    ...
    async def on_job_done(self, kind: str, take: int, result: dict):
        ...
        if not call_id:
            return
        if result["qc"] == "passed":
            # TODO(HOOK 3): the join. Emit `job_completed` (job, asset_url,
            # take), patch the asset to done, increment self.done_jobs and
            # emit `join_progress` {done, total: TOTAL_JOBS}. The join is
            # counted HERE, in the driver — never in the model.
            pass
